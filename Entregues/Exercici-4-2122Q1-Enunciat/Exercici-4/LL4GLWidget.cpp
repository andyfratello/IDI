...
glm::mat4 LL4GLWidget::calculaPatricioTG()
{
  glm::mat4 tg = glm::translate(glm::mat4(1.f), glm::vec3(5,0,2));
...
{
  terraTG = glm::mat4(1.f);  // Matriu de transformació
  glUniformMatrix4fv (transLoc, 1, GL_FALSE, &terraTG[0][0]);
}
...
{
  View = glm::translate(glm::mat4(1.f), glm::vec3(0, 0, -16));
  View = glm::rotate(View,  angleX, glm::vec3(1, 0, 0));
  View = glm::rotate(View, -angleY, glm::vec3(0, 1, 0));
  View = glm::translate(View, glm::vec3(-5, -3, -5));
...
void LL4GLWidget::calculaCapsaModel ()
{
  ...
  escala = 3.0/(maxy-miny);
  centreBasePatr[0] = (minx+maxx)/2.0; 
  centreBasePatr[1] = miny; 
  centreBasePatr[2] = (minz+maxz)/2.0;
  dimCapsaPatr[0] = maxx-minx;
  dimCapsaPatr[1] = maxy-miny;
  dimCapsaPatr[2] = maxz-minz;
}

void LL4GLWidget::creaBuffersPatricio ()
{
...
void LL4GLWidget::creaBuffersTerraIParet ()
{
  // VBO amb la posició dels vèrtexs
  glm::vec3 posterra[18] = {
  ...
  glm::vec3 norm3 (1,0,0);
  glm::vec3 normterra[18] = {
    norm1, norm1, norm1, norm1, norm1, norm1, // la normal (0,1,0) per als primers dos triangles (terra)
    norm2, norm2, norm2, norm2, norm2, norm2, // la normal (0,0,1) per als dos seguents triangles (paret 1)
    norm3, norm3, norm3, norm3, norm3, norm3  // la normal (1,0,0) per als dos darrers triangles (paret 2)
  };

  iniMaterialTerra();

  // Fem que el material del terra afecti a tots els vèrtexs per igual
  glm::vec3 matambterra[18] = {
    amb, amb, amb, amb, amb, amb, amb, amb, amb,
    amb, amb, amb, amb, amb, amb, amb, amb, amb
  };
  glm::vec3 matdiffterra[18] = {
    diff, diff, diff, diff, diff, diff, diff, diff, diff,
    diff, diff, diff, diff, diff, diff, diff, diff, diff
  };
  glm::vec3 matspecterra[18] = {
    spec, spec, spec, spec, spec, spec, spec, spec, spec,
    spec, spec, spec, spec, spec, spec, spec, spec, spec
  };
  float matshinterra[18] = {
    shin, shin, shin, shin, shin, shin, shin, shin, shin,
    shin, shin, shin, shin, shin, shin, shin, shin, shin
  };
